...
| # | Milestone | Output | Effort |
|---|---|---|---|
| **S1** | **Texture A/B at fixed geometry**: seeds {42, +2}, `--texture 0.0` vs `1.0` (binary in effect: uniform vs patchy ground compositor) at same seed/layout/route → bundle immediately → `m3-smoke` → `m4_lio_eval.sh`; probe mid‑route corner counts (`check_sim_texture.py`) to evidence the *mechanism*. Hand‑loop the 6 worlds — don't wait for S3. | RPE chart: VIO degrades on uniform ground, LIO flat, geometry held constant | 1 session |
